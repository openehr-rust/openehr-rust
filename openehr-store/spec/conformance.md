...
and then Verified the same day, 2026-09-12: real locally first
(`scripts/verify-store.sh`), then a green CI run to cite — run
[34710118312](https://github.com/openehr-rust/openehr-rust/actions/runs/34710118312)
— which is what Verified itself requires.
...
| `openehr-postgresql` | **Verified** | `PostgresqlStore` implements `Store` against **PostgreSQL 18**: the same three conformance suites `openehr-sqlite` runs, plus concurrency, tamper-chain, and checkpoint tests ported from it, all passing against a real, disposable server (`scripts/verify-store.sh`), and re-checked in CI on every push (run 34710118312, 2026-09-12). Mutation-tested: 35/39 viable diff mutants caught. | `db:D-14`: no keyed-chain or two-system-race test — a gap in the shared conformance fixtures, not in this engine. |
