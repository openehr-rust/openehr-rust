# openehr-postgresql-fuzz

Fuzz targets for the **PostgreSQL** dialect of [`openehr-postgresql`](../openehr-postgresql).
...
PostgreSQL quotes identifiers as `"openehr_version"`, escaping an embedded `"` by
doubling it to `""`.

PostgreSQL folds unquoted identifiers to lower case, so this schema quotes every one — which means every identifier passes through the escaping this target drives.
...
cargo +nightly fuzz run --fuzz-dir ../openehr-postgresql-fuzz col_sql -- -max_total_time=60
```
...
```sh
cargo +nightly fuzz run --fuzz-dir ../openehr-postgresql-fuzz quote artifacts/quote/<file>
```
