...
| Not implemented | Why |
| --- | --- |
| A whole-archetype ADL parser, ADL 1.4 bodies, flattening, templates, OPT | **specified, not built.** `S1.4` excluded the Archetype Model until 2026-08-26; [§15](spec/15-archetypes.md) now requires it. `am` is the AOM2 object model (`K15.1`–`K15.4`), `am::validate` checks an instance against an archetype already in memory (`K15.18`–`K15.23`), and `am::cadl` reads a `definition` section (774 of 1,379 published ADL 2 files as of 2026-09-03, [`spec/corpus.md`](spec/corpus.md)); 16 of §15's 33 requirements have no code (`A-40`), so a composition that passes `validate()` may still violate the *published* archetype it names. A partial constraint engine stays prohibited — `K15.20` refuses rather than passes |
