...
Fuzz targets for the **SQL Server** dialect of [`openehr-mssql`](../openehr-mssql).
...
SQL Server quotes identifiers as `[openehr_version]`, escaping an embedded `]` by
doubling it to `]]`.

SQL Server is the only engine here whose delimiters differ from each other. Only `]` is special inside brackets; a `[` is passed through unescaped, which is correct T-SQL and a case worth fuzzing precisely because it looks like an oversight.
...
cd ../openehr-mssql
cargo +nightly fuzz run --fuzz-dir ../openehr-mssql-fuzz quote   -- -max_total_time=60
cargo +nightly fuzz run --fuzz-dir ../openehr-mssql-fuzz col_sql -- -max_total_time=60
```
...
```sh
cargo +nightly fuzz run --fuzz-dir ../openehr-mssql-fuzz quote artifacts/quote/<file>
```
