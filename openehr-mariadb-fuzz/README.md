# openehr-mariadb-fuzz

Fuzz targets for the **MariaDB** dialect of [`openehr-mariadb`](../openehr-mariadb).
...
MariaDB quotes identifiers as ``openehr_version``, escaping an embedded ``` by
doubling it to ``` `` ```.

MariaDB quotes as MySQL does. The two dialects are separated by index idempotence and trigger syntax, not by quoting — so this target is one of the places they legitimately agree, and the crate-level tests assert the places they must not.
...
cd ../openehr-mariadb
cargo +nightly fuzz run --fuzz-dir ../openehr-mariadb-fuzz quote   -- -max_total_time=60
cargo +nightly fuzz run --fuzz-dir ../openehr-mariadb-fuzz col_sql -- -max_total_time=60
```
...
```sh
cargo +nightly fuzz run --fuzz-dir ../openehr-mariadb-fuzz quote artifacts/quote/<file>
```
