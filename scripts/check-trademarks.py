...
- `llms.txt`, root-level like the `*.md` files above but a `.txt` extension
  the glob does not match, added explicitly rather than left for a reader to
  assume it is covered. It is written *for* AI tools to read without a
  human first — exactly the audience the notice exists to reach, so it is
  not exempt the way `AGENTS.md`/`CLAUDE.md` are.
- `llms.json`'s `trademark_notice` field: structured data rather than prose,
  so the prose scanner does not reach it (the same reason `description` above
  gets its own check rather than the generic one). Required to equal the
  notice exactly, the same shape `check_descriptions` holds `Cargo.toml` to.
...
def check_llms_json() -> int:
    """`llms.json`'s `trademark_notice` field equals the notice exactly.

    Structured data, not prose (see the module docstring's Scope section) —
    checked the same way `check_descriptions` holds `Cargo.toml` to a shape
    rather than scanning it for the mark.
    """
    path = ROOT / "llms.json"
    if not path.is_file():
        # `llms.json` is generated by `scripts/generate-llms-files.py`, which
        # has its own existence check; nothing to report twice here.
        return 0
    data = json.loads(path.read_text(encoding="utf-8"))
    if data.get("trademark_notice") != NOTICE:
        print(
            f"::error file={path.relative_to(ROOT)}::`trademark_notice` is "
            "missing or does not match the notice verbatim "
...
def main() -> int:
    ...
    if llms_txt.is_file():
        targets.append((llms_txt, "md"))

    bad = max(check_descriptions(), check_llms_json())
    uses = 0
