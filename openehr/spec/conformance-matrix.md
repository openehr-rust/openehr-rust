...
| K15.5 | spec | ADL 2 in full. `am::adl2::parse_header` (the header) and `am::cadl::parse_definition` (`definition`'s own `c_complex_object` — every node kind but `SIBLING_ORDER`, `A-62`–`A-67`, `A-72`–`A-76`) read two sections and do not compose into an `Archetype`: no `language`, `description`, `rules`, `terminology`, or `annotations` — **A-40** |
