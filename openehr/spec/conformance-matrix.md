...
| withdrawn | 1 | 1 | 1 | — |
| open | 0 | 0 | 0 | 4 |
| **total requirements** | **344** | **344** | **344** | 269 |
...
| S1.21 | spec | §15 in full — sixteen of the thirty-two ids it brought into scope are **•** and sixteen **spec** (2026-09-03); **A-40** |
...
The section was added on 2026-08-26 when `S1.4` was withdrawn. **Sixteen ids
are satisfied and sixteen are not**: `openehr::am` is the AOM2 object model,
...
| K15.6–K15.7 | • | `am::cadl::tests::occurrences_omitted_on_a_non_root_node_is_refused_naming_it`, `…a_closed_archetype_slot_is_refused_by_name`, `…a_slot_assertion_using_a_quoted_string_instead_of_a_regex_is_refused`, `…an_unterminated_contained_regexp_is_refused_naming_it`, `…a_c_attribute_tuple_with_an_unwrapped_interval_item_hits_a_different_refusal`, `…trailing_content_after_the_root_is_refused`, `…a_real_published_archetypes_definition_is_refused_by_name_not_mis_parsed`; `am::adl2::tests::a_trailing_concept_section_is_refused_not_silently_accepted`; `am::adl14::tests::missing_concept_section_is_refused_not_defaulted`. **Scope:** proven for the parsers that exist — `am::cadl` and the two header readers — each refusing an unimplemented construct by name at its offset and returning no partial tree; binding on any parser `K15.5` later adds |
