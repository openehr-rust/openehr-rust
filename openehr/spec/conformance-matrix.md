...
| T13.2 | ? | `cargo-mutants` over four modules: `audit_chain` 40→1, `integrity` 15→0, `record` 4→0, `dialect` 25→5. The recurring cause is code whose tests live in another crate, which `cargo mutants` does not run. Roughly twenty more checks mutated by hand. Not in CI, and most modules are untouched — **A-09** |
