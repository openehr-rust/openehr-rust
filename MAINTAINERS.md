...
| A crates.io API token, used from a workstation | the eight published crates | the maintainer, on his own machine — **and, since 2026-09-02, a Claude Code session running on that same machine, invoked by the maintainer, under that same already-authenticated `cargo login`.** Not a second identity: no separate token exists, the crates.io owner account is unchanged, and the accountability for what gets published stays the maintainer's. **There is still no Trusted Publishing and no CI publish workflow** — `.github/workflows/` holds `ci.yml` and nothing else, and `agents/publishing.md` documents publishing as a manual `cargo publish` after `cargo login`, which a Claude Code session now performs under the same rules a human running it would follow (see that file's own governance-change note). Stated policy, not an oversight: [`spec/trusted-publishing/index.md`](spec/trusted-publishing/index.md) adopts Trusted Publishing once it is production-ready across every forge this repository actually pushes to (GitHub, GitLab, Codeberg — all three real remotes) and every destination it publishes to (crates.io) | none. A leaked token is revoked at crates.io; a lost one is reissued by the same account. Crate ownership moves only by `cargo owner`, which needs that account |
