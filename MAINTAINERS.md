...
| A crates.io API token, used from a workstation | the eight published crates | the maintainer, on his own machine. **There is no Trusted Publishing and no publish workflow** — `.github/workflows/` holds `ci.yml` and nothing else, and `agents/publishing.md` documents publishing as a manual `cargo publish` after `cargo login`. Stated policy, not an oversight: [`spec/trusted-publishing/index.md`](spec/trusted-publishing/index.md) adopts Trusted Publishing once it is production-ready across every forge this repository actually pushes to (GitHub, GitLab, Codeberg — all three real remotes) and every destination it publishes to (crates.io) | none. A leaked token is revoked at crates.io; a lost one is reissued by the same account. Crate ownership moves only by `cargo owner`, which needs that account |
