...
| `supply-chain` | `cargo deny check` and `cargo audit`, against all eighteen crates including the eight fuzz crates, one shared `deny.toml` at the repository root — added 2026-09-06 to close **SECURITY.md**'s own "no SBOM, no cargo-deny/audit" line |
...
parallel implementation in YAML.

**Update, 2026-09-06.** The sentence that stood here — "There is deliberately
