...
| `claims` | that no engine crate claims Schema or above without a `schema` job backing it (the check list emptied 2026-09-06, once oracle and then mssql each got one), that the library matrix covers every requirement exactly once, that the conformance matrix does not contradict itself, that the audit summary counts itself correctly, that every one of the 221 `spec/databases/` requirements is mentioned in the matrix at least once (`db:D-11`, closed 2026-09-10), that `openehr/spec/corpus-index.md` (a reverse index from a requirement or finding id to where a corpus run cites it) is current, and that **all eighteen** crates declare the same five licences (**W-14**) |
...
| `mutants` | `cargo-mutants --in-diff` over the lines a **push or a pull request** changed, per crate touched. It was pull-request-only until 2026-08-21, so nine commits made straight to `main` bypassed it entirely while it reported `skipped` (**W-18**). A push is mutated against `event.before..HEAD` only, so a survivor in code an earlier push carried is never re-checked here; run it locally by function name before pushing — see [`agents/auditing.md`](agents/auditing.md). `openehr-sqlite` runs with `--in-place` rather than the isolated copy every other crate uses — its five sibling dev-dependencies (`W-01`'s own comparison) do not resolve inside that copy, which failed the build rather than the mutants for as long as no push happened to touch this crate's own mutable `src/` lines directly (**W-20**) |
