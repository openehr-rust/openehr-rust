...
Fuzz targets for the **Oracle Database** dialect of [`openehr-oracle`](../openehr-oracle).
...
Oracle Database quotes identifiers as `"openehr_version"`, escaping an embedded `"` by
doubling it to `""`.

Oracle folds unquoted identifiers to UPPER case, so this schema quotes every one to keep its lower-case names identical to the other five engines' (`X15.3`).
...
cd ../openehr-oracle
cargo +nightly fuzz run --fuzz-dir ../openehr-oracle-fuzz quote   -- -max_total_time=60
cargo +nightly fuzz run --fuzz-dir ../openehr-oracle-fuzz col_sql -- -max_total_time=60
```
...
```sh
cargo +nightly fuzz run --fuzz-dir ../openehr-oracle-fuzz quote artifacts/quote/<file>
```
