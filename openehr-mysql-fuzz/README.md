...
Fuzz targets for the **MySQL** dialect of [`openehr-mysql`](../openehr-mysql).
...
MySQL quotes identifiers as ``openehr_version``, escaping an embedded ``` by
doubling it to ``` `` ```.

MySQL's backtick quoting is not SQL-standard, which is exactly why it needs its own target: an identifier containing a backtick is escaped by doubling, and nothing else in the statement is.
...
cd ../openehr-mysql
cargo +nightly fuzz run --fuzz-dir ../openehr-mysql-fuzz quote   -- -max_total_time=60
cargo +nightly fuzz run --fuzz-dir ../openehr-mysql-fuzz col_sql -- -max_total_time=60
```
...
```sh
cargo +nightly fuzz run --fuzz-dir ../openehr-mysql-fuzz quote artifacts/quote/<file>
```
